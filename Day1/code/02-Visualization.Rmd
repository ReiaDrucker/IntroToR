...
```{r}
ggplot(data = bechdel) + geom_boxplot(mapping = aes(x = clean_test, y = budget))
```
...
```{r}
ggplot(data = bechdel) + geom_histogram(mapping = aes(x = budget))
```
...
```{r}
ggplot(data = bechdel) + geom_histogram(mapping = aes(x = budget), binwidth = 10000000)
```
